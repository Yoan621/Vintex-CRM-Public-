import { NextRequest, NextResponse } from 'next/server'
import { prisma } from '@/lib/db/prisma'
import { withAuthAndRateLimit } from '@/lib/auth/extension-auth'
import { validateVentes, formatZodError } from '@/lib/validation/sync-validators'
import { z } from 'zod'

/**
 * POST /api/sync/ventes
 * Synchronise les ventes depuis l'extension Chrome
 */
export async function POST(request: NextRequest) {
  return withAuthAndRateLimit(request, async (req) => {
    const startTime = Date.now()

    try {
      // Parser le body
      const body = await req.json()

      // Valider les données avec Zod
      let ventes
      try {
        ventes = validateVentes(body.ventes)
      } catch (error) {
        if (error instanceof z.ZodError) {
          return NextResponse.json(
            {
              success: false,
              error: 'Validation error',
              details: formatZodError(error)
            },
            { status: 400 }
          )
        }
        throw error
      }

      let created = 0
      let updated = 0
      const errors: any[] = []

      // Traiter chaque vente
      for (const venteData of ventes) {
        try {
          // Chercher si la vente existe déjà
          const existing = await prisma.vente.findUnique({
            where: { transactionNumber: venteData.transactionNumber }
          })

          if (existing) {
            // Détecter les changements significatifs
            const hasChanges =
              existing.status !== venteData.status ||
              existing.trackingNumber !== venteData.trackingNumber ||
              existing.salePrice !== venteData.salePrice ||
              existing.validationDate?.getTime() !== venteData.validationDate?.getTime() ||
              existing.shippingDate?.getTime() !== venteData.shippingDate?.getTime()

            if (hasChanges) {
              // Mettre à jour
              await prisma.vente.update({
                where: { id: existing.id },
                data: {
                  ...venteData,
                  updatedAt: new Date(),
                  syncedAt: new Date(),
                  syncSource: 'extension'
                }
              })
              updated++
            }
          } else {
            // Créer une nouvelle vente
            await prisma.vente.create({
              data: {
                ...venteData,
                profit: venteData.salePrice - venteData.purchasePrice,
                syncSource: 'extension'
              }
            })
            created++
          }
        } catch (error) {
          console.error('Erreur traitement vente:', error)
          errors.push({
            transactionNumber: venteData.transactionNumber,
            error: error instanceof Error ? error.message : 'Erreur inconnue'
          })
        }
      }

      const durationSeconds = Math.round((Date.now() - startTime) / 1000)

      // Logger la synchronisation
      await prisma.syncLog.create({
        data: {
          syncType: 'ventes',
          status: errors.length === 0 ? 'success' : (ventes.length === errors.length ? 'error' : 'partial'),
          itemsProcessed: ventes.length,
          itemsCreated: created,
          itemsUpdated: updated,
          itemsErrors: errors.length,
          source: 'extension',
          durationSeconds,
          errorDetails: errors.length > 0 ? JSON.stringify(errors) : null,
          endTime: new Date()
        }
      })

      // Mettre à jour la date de dernière synchronisation
      const apiKey = request.headers.get('X-API-Key')
      if (apiKey) {
        await prisma.extensionConfig.updateMany({
          where: { apiKey },
          data: { lastSyncVentes: new Date() }
        })
      }

      return NextResponse.json({
        success: true,
        created,
        updated,
        errors,
        durationSeconds
      })

    } catch (error) {
      console.error('Erreur API sync ventes:', error)

      // Logger l'erreur
      await prisma.syncLog.create({
        data: {
          syncType: 'ventes',
          status: 'error',
          itemsProcessed: 0,
          source: 'extension',
          errorMessage: error instanceof Error ? error.message : 'Erreur inconnue',
          durationSeconds: Math.round((Date.now() - startTime) / 1000),
          endTime: new Date()
        }
      })

      return NextResponse.json(
        {
          success: false,
          error: error instanceof Error ? error.message : 'Erreur inconnue'
        },
        { status: 500 }
      )
    }
  })
}
